...
# os tweets do arquivo e seus pesos + contador de linhas
...
# Funcao que dado uma string (um tweet nesse contexto), simplifica da seguinte forma:
    ...
    # elimina todas as palavras com tamanho < 3
def reduceTT(tweet):
    # deixa todos os caracteres minusculos
    tweet = tweet.lower()
...
def polarizeTweet(filename, outPolarize, dicio):
    # Começamos criando o arquivo "TweetsPolarizados.csv" para armazenar os
    # tweets polarizados
    outFile = open(outPolarize, 'w', encoding="utf8")
...
def funcionalidadeA(searchWord, root, postings, inFile, outFuncA):
    with open(outFuncA, 'w', encoding="utf8") as csvFile:
...
def funcionalidadeB(prefix, root, outFuncB):
    getByPrefix(root, prefix, 0, outFuncB)
